...
// @desc - Get all users, optionally filtered by exact billno, or by a
//         createdAt date range (defaults to the current month)
// @route GET /users
// @access Private
const getAllLedger = asyncHandler(async (req, res) => {
    const { billno, startDate, endDate } = req.query

    let filter
    if (billno) {
        filter = { billno: { $regex: `^${escapeRegex(billno)}$`, $options: 'i' } }
    } else {
        const now = new Date()
